...
import { useAuthStatusCheck } from '@/hooks/admin/useAuthStatusCheck';
import { supabase } from '@/integrations/supabase/client';
import { Plus, Bot, Edit, Trash2, Link, FileText } from 'lucide-react';
...
const AgentesIA = () => {
  ...
  useEffect(() => {
    console.log("AgentesIA: Auth status check - isLoggedIn:", isLoggedIn, "isAdmin:", isAdmin, "currentAuthStatus:", currentAuthStatus);
    
    // Aguardar todos os valores estarem definidos
    if (isLoggedIn === undefined || isAdmin === undefined) {
      console.log("AgentesIA: Still waiting for auth status...");
      return;
    }

    // Marcar que a verificação de auth foi feita
    setAuthChecked(true);
    
    if (!isLoggedIn) {
      console.log("AgentesIA: User not logged in, redirecting to login");
      navigate('/login');
      return;
    }
    
    if (!isAdmin) {
      console.log("AgentesIA: User not admin, redirecting to dashboard");
      toast({
        title: "Acesso negado",
    ...
    console.log("AgentesIA: User is authenticated admin, fetching agents...");
    fetchAgentes();
                  ...
                  <div className="space-y-3">
                    ...
                    {agente.token_id && (
                      <div className="text-xs text-muted-foreground">
                        Token ID: {agente.token_id.substring(0, 8)}...
                      </div>
                    )}
